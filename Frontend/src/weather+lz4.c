...
byte *OpenFile(char *fileName, int *XY)
{
	...
	else
	{
		...
		int ySize = 0;
		int twoFive=0;
		fscanf(fp,"%c%c%c%d%c%d%c%d%c",&one,&two,&three,&xSize,&five,&ySize,&five,&twoFive,&six);
		XY[0] = xSize;
		XY[1] = ySize;
		//printf("\nOpenfile: %d %d %d",XY[0],XY[1],twoFive);
...
float **CreateMCUS(byte *buffer, int *XY, int dataLength)
{
	...
	{
		printf("\nError: Width: %d or height: %d is not a multiple of 8\n", (XY[0] % 8),(XY[1] % 8));
		return NULL;
	}
...
void PackageAndSendFrame(char *fileName)
{
	...
	//Write to file
	FILE *fp = fopen("bicho2.bae", "wb");
	fwrite(&quantizationLevel,4,1,fp);
	fwrite(&compressedDataSize,4,1,fp);
	fwrite(&charArrayLength,1,sizeof(charArrayLength),fp);
...
int UnpackageAndReceiveFrame(char *fileName, char *pgmName, char *receivedFileLocation)
{
	...
	fread(&charArrayLength,1,sizeof(charArrayLength),fp);
...
int main()
{
	...
	char *bitmapName = "a.bmp";
	char *pgmName = "a.pgm";
	char *receivedFileLocation = "bicho2.bae";
	UnpackageAndReceiveFrame(bitmapName,pgmName,receivedFileLocation);
	return 0;
}*/
